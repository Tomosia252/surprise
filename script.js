document.addEventListener('DOMContentLoaded', function() {
    ...
    // ELEMEN HTML (ID DIPASTIKAN COCOK)
    ...
    // FASE 1: TEKS AWAL (PERBAIKAN ANTI HILANG/STUCK)
    if (textArea) {
        textArea.addEventListener('click', function() {
            playBackgroundMusic(0.3); 
            
            // Sembunyikan teks saat ini perlahan
            messageEl.classList.add('text-hidden');
            
            setTimeout(function() {
                if (msgIndex < greetings.length) {
                    messageEl.innerText = greetings[msgIndex];
                    messageEl.classList.remove('text-hidden'); // Tampilkan teks baru
                    msgIndex++;
                } else {
    ...
    // FASE 2: TOMBOL SIAPP -> PUISI -> BUNGA -> CHAT
    if (btnYes) {
        btnYes.addEventListener('click', function() {
            playBackgroundMusic(0.3);
            contentContainer.style.opacity = '0'; // Hilangkan layar awal
            
            setTimeout(function() {
                contentContainer.style.display = 'none';
                
                // MUNCULKAN PANEL PUISI
                if (poemPanel) {
                    poemPanel.style.display = 'flex';
                    setTimeout(function() { poemPanel.classList.add('show'); }, 50);

                    // TUNGGU 12 DETIK LALU GANTI KE BUNGA
                    setTimeout(function() {
    ...
    // FASE 3: MUNCULKAN CHAT FULLSCREEN
    ...
    function addMessage(side, type, content) {
        ...
        if (type === 'audio') {
            msgDiv.innerHTML = '<audio controls><source src="' + content + '" type="audio/mpeg"></audio>';
             ...
             if (content === 'bypass') { msgDiv.innerHTML = '<audio controls></audio>'; } 
    ...
    if (btnSend) {
        btnSend.addEventListener('click', function() {
            ...
            previewControls.style.display = 'none'; inputStatus.innerText = "Pesan terkirim!";
            ...
            setTimeout(function() {
                ...
                setTimeout(function() {
                    ...
                    setTimeout(function() { window.location.href = "minigame.html"; }, 2500);
                }, 2000);
            }, 1000);
        });
    }
